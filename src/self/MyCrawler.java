...
public class MyCrawler extends WebCrawler {
	...
	private final static Pattern FILTERS = Pattern.compile(".*(\\.(css|js|bmp|gif|jpe?g"
			...
			+ "|rm|smil|wmv|swf|wma|zip|rar|gz"+"doc|docx|bib|ppt|pptx"+"))$");
	//private final static Pattern IMAGE_FILTERS = Pattern.compile(".*(|png|tiff?|mid|mp2|mp3|mp4))$");
...
static int count;
	...
	public void visit(Page page) {
		...
		if (page.getParseData() instanceof HtmlParseData) {
			...
			if(null!=cCrawlStat.getLinks()){
				links=cCrawlStat.getLinks(); //= htmlParseData.getOutgoingUrls();
			}else{
				links=new ArrayList<WebURL>();
			}
			if(null!=cCrawlStat.getIndexableData()){
				indexableData=cCrawlStat.getIndexableData(); //= htmlParseData.getOutgoingUrls();
			}else{
				links=new ArrayList<WebURL>();
			}
